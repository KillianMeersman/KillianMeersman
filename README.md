# Hello! 👋

I'm Killian: Software engineer, chronic overthinker & start-up addict.

## I do...
- **Software Engineering**: Software architecture and coding in Python, Javascript (Vue & React), Go & Rust. 
- **DevOps**: Set up infrastructure using IaC, monitoring using OpenTelemetry, Grafana stack; Both on-prem & in the cloud.
- **Platform Engineering**: Built a self-service platform that manages deployments to Kubernetes, serverless & DNS records to automate DevOps workloads @ Just Russel. 

## Currently working on...
- To be announced 🥷

## Have worked on...
- **[Just Russel](https://justrussel.com) • Founding Engineer**: Helped Just Russel scale from tiny start-up to to ~50M ARR.
- **[TransDirect](https://transdirect.be) • Freelance Software Engineer**: Enhanced TransDirect's transport management system with a carrier portal.
- **[uman.ai](https://www.uman.ai/) • Founding Engineer**: Built the first iteration of their platform through multiple pivots until PMF.

## Want to talk?
Contact me using my email or the contact form on my website, I'll get back to you ASAP.
